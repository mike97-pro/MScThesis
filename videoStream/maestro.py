...
class Controller:
    ...
    def setRangeDeg(self, chan, min, max):
        self.MinsDeg[chan] = min
        self.MaxsDeg[chan] = max
    ...
    def setTargetDegree(self, chan, target):
        if self.Mins[chan] == 0 and self.Maxs[chan] == 0:
            return
        if self.MinsDeg[chan] == 0 and self.MaxsDeg[chan] == 0:
            return
        # if Min is defined and Target is below, force to Min
        if self.MinsDeg[chan] > 0 and target < self.MinsDeg[chan]:
            target = self.MinsDeg[chan]
        # if Max is defined and Target is above, force to Max
        if self.MaxsDeg[chan] > 0 and target > self.MaxsDeg[chan]:
            target = self.MaxsDeg[chan]
            
        t = (target-self.MinsDeg[chan])*(self.Maxs[chan]-self.Mins[chan])/(self.MaxsDeg[chan]-self.MinsDeg[chan])+self.Mins[chan]
        self.setTarget(chan,int(t))
